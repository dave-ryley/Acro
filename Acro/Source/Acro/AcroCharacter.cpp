...
void AAcroCharacter::Tick(float DeltaSeconds)
{
	...
	curLocation = (curLocation / distanceCheck.Size()) * RADIUS;
...
void AAcroCharacter::SetupPlayerInputComponent(class UInputComponent* PlayerInputComponent)
{
	...
	PlayerInputComponent->BindAxis("Move2D", this, &AAcroCharacter::Move2D);
...
void AAcroCharacter::Move2D(float Value)
{
	// add movement in that direction
	if (Value != 0.0f)
	{
		FVector curLocation = GetActorLocation();
		float rotationAmount = -Value / CIRCUMFERENCE * 360.f;
