...
FString AcroRemoteResourceManager::GenerateUUID()
{
...
bool AcroRemoteResourceManager::DeleteAcroMesh(FLevelData * LevelData, UAcroMesh* Mesh)
{
	if (!GorbClient.IsConnected()) return false;
	return GorbClient.DeleteAcroMesh(LevelData->UUID, Mesh->UUID);
}